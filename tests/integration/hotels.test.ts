...
describe("GET /hotels", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 when user has a paid ticket that is remote", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType();
    ...
    it("should respond with status 404 when user has a paid ticket that doesnt include hotel", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType();
